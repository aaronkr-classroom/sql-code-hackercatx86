...
CREATE VIEW v1_고학년학생 AS SELECT 이름, 나이, 성별, 학년 FROM 학생2 WHERE 학년 >= 3 AND 학년 >= 4;

CREATE VIEW v2_과목수강현황(과목번호, 강의실, 수강인원수) AS SELECT 과목.과목번호, 강의실, COUNT(과목.과목번호) 
	FROM 과목 JOIN 수강 ON 과목.과목번호 = 수강.과목번호 GROUP BY 과목.과목번호;

CREATE VIEW v3_고학년여학생 AS SELECT * FROM v1_고학년학생 WHERE 성별 = '여';
